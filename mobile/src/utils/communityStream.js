...
export function createCommunitySubscription({
  apiUrl,
  token,
  onEvent,
  onError,
  ...
  async function startPollingFallback() {
    const poll = async () => {
      try {
        const res = await fetch(`${apiUrl}/api/v1/community/unread`, {
          headers: {
    ...
    await poll();
    pollingTimer = setInterval(poll, 10_000);
  }

  async function startStream() {
    try {
      const res = await fetch(`${apiUrl}/api/v1/community/live`, {
        method: 'GET',
        headers: {
          Authorization: `Bearer ${token}`,
